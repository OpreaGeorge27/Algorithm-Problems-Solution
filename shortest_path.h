...
struct DijkstraResult {
  vector<double> d;
  vector<int> p;
  DijkstraResult(const vector<double> &d, const vector<int> &p) : d(d), p(p) {}
};
...
struct JohnsonResult {
  ...
  JohnsonResult(bool has_cycle, const vector<vector<int>> &d,
...
DijkstraResult dijkstra(int source, int nodes,
  ...
  // distanta minima.
  set<pair<double, unsigned long long int>> pq;
